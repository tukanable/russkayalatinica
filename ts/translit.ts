...
enum ContextType {
  ...
  NonOther = 8,
  Letter = 9,
  HardSign = 10, // Special context for hard sign
}
...
const elementTypeMap: Map<string, ElementType> = new Map([
  // Vowels
...
function buildScriptTable(): Cell[] {
...
function contextMatches(ctx: ContextType, elementType: ElementType, prevChar: string): boolean {
  switch (ctx) {
...
function getElementType(char: string): ElementType {
  if (!isLetter(char)) {
...
function isFollowedByUppercase(chars: string[], pos: number): boolean {
...
function isPrecededByUppercase(chars: string[], pos: number): boolean {
  if (pos === 0) {
    return false;
  }
  return chars[pos - 1] === chars[pos - 1].toUpperCase() && isLetter(chars[pos - 1]);
...
function applyProperCase(
  ...
  followedByUpper: boolean,
  precededByUpper: boolean
): string {
  if (!isUppercase) {
    return input;
  }
...
function buildReverseTable(): Map<string, string> {
 ...
 */
export function revertTranslit(input: string): string {
  if (!input) {
    return "";
  }
...
// Export additional types for use in tests or other modules
export { ElementType, ContextType, Cell };